...
OUTPUT_JSON = "/data/zkx/zkx/iknow-audio/27.residual_moe/tut2017/results_residual_moe.json"
...
def residual_moe_fusion(base_score, hop1_score, hop2_score, temperature):
    ...
    delta1 = max(0.0, hop1_value - base_value) if hop1_value is not None else 0.0
    delta2 = max(0.0, hop2_value - base_value) if hop2_value is not None else 0.0
    g_h1 = 0.0 if hop1_value is None else 1.0 / (1.0 + np.exp(-((hop1_value - base_value) / temp)))
    g_h2 = 0.0 if hop2_value is None else 1.0 / (1.0 + np.exp(-((hop2_value - base_value) / temp)))

    if delta1 <= 0.0 and delta2 <= 0.0:
        return base_score, (1.0, 0.0, 0.0)

    w1 = g_h1 if delta1 > 0.0 else 0.0
    w2 = g_h2 if delta2 > 0.0 else 0.0
    total = w1 + w2
    if total <= 1e-8:
        return base_score, (1.0, 0.0, 0.0)
    ...
    w1 /= total
    w2 /= total
    fused_value = base_value + (w1 * delta1) + (w2 * delta2)
    fused = torch.tensor(fused_value, device=base_score.device, dtype=base_score.dtype)
    return fused, (1.0, w1, w2)


def run_selective2hop_residual_moe(clap_model, audio_embed, cos_sim_orig, top_indices, label_classes, kg_classes, class_labels_set, hop1_relations, hop2_relations, get_tails_fn, prompt_map, temperature):
    score = cos_sim_orig.clone()
    prompt_count = 0
    hop2_flags = []
    knowledge_weights = []
    for ci in top_indices:
        ...
        fused_score, gate_values = residual_moe_fusion(cos_sim_orig[ci], hop1_s, hop2_s, temperature)
...
@torch.no_grad()
def collect_residual_temperature(clap_model, dataset, text_embeds, label_classes, kg_classes, class_labels_set, hop1_relations, hop2_relations, get_tails, prompt_map):
    deltas = []
    skipped = 0
    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="Collect TUT residual deltas"):
...
def print_final_tables(results):
    ...
    order = ["Baseline", "iKnow", "Full2Hop", "Selective2Hop_OriginalAgg", "Selective2Hop_StaticAlpha", "Selective2Hop_ResidualMoE"]
    print("\n" + "=" * 180)
    print(f"{'Metric':<8} | {'Baseline':<10} | {'iKnow':<10} | {'Full2Hop':<12} | {'Sel2 OriginalAgg':<18} | {'Sel2 StaticAlpha':<18} | {'Sel2 ResidualMoE':<18}")
    print("-" * 180)
...
@torch.no_grad()
def main():
    print("Starting TUT2017 residual-MoE fusion pilot")
    print(f"Device: {DEVICE}")
    print("Prompt policy: baseline label-only, iKnow uses pure single-hop concat without Top-P, selective hop1/hop2 use LLM prompts with concat fallback; final fusion preserves baseline and adds only positive hop1/hop2 residual gains")
    ...
    moe_temperature, gain_skipped = collect_residual_temperature(
        clap_model, dataset, text_embeds, label_classes, kg_classes,
        class_labels_set, hop1_relations, hop2_relations, get_tails, prompt_map
    )
    print(f"Residual-MoE temperature: temp={moe_temperature:.6f}, skipped={gain_skipped}")
    results = init_results()
    skipped_count = 0

    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="TUT2017 residual MoE"):
        audio_path = sample["audio_path"]
        true_indices = sample["true_indices"]
        if not os.path.exists(audio_path) or not true_indices:
            skipped_count += 1
        ...
        t = time.time()
        score, prompt_count, extras = run_selective2hop_residual_moe(
        ...
        results["Selective2Hop_ResidualMoE"]["ranks"].append(rank_of_true(score, true_indices))
        results["Selective2Hop_ResidualMoE"]["times"].append(audio_ms + (time.time() - t) * 1000.0)
        results["Selective2Hop_ResidualMoE"]["prompts"].append(prompt_count)
        results["Selective2Hop_ResidualMoE"]["hop2_activation_sample"].append(bool(extras["hop2_activated"]))
        results["Selective2Hop_ResidualMoE"]["hop2_activation_candidate"].append(float(extras["candidate_level_activation_rate"]))
        results["Selective2Hop_ResidualMoE"]["alphas"].append(float(extras["alpha_mean"]))
