...
# ==========================================
# 3. 主程序：多跳 + LLM自然语义 + M3聚合公式
# ==========================================
@torch.no_grad()
def main():
    print("🚀 启动 TUT2017 终极版: [多跳图谱] + [LLM自然语义] + [M3衰减聚合]...")
    ...
    col_w = 22
    header = f"{'Metric':<8} | {'Baseline':<{col_w}} | {'M3-RAG + LLM 增强':<{col_w}}"
    ...
    print("\n💡 终极版机制解析 (TUT2017):")
    print("1. 图谱层：通过二跳解绑 'scene contains'，防止场景概念无限泛化。")
    print("2. 语义层：引入 LLM，将干瘪的声学物体还原为生动的场景意象。")
    print("3. 护航层：采用 M3 机制 (Decay 0.85 + LogSumExp + Alpha 0.6)，完美过滤掉 LLM 过度发散的幻觉描述。")
