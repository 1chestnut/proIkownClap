...
# --- 修改点：重命名变量，避免覆盖类名 ---
bert_tokenizer_local = BertTokenizer.from_pretrained(
    "/home/star/zkx/CLAP/model/bert-base-uncased",
    local_files_only=True
)

roberta_tokenizer_local = RobertaTokenizer.from_pretrained(
    "/home/star/zkx/CLAP/model/roberta-base",
    local_files_only=True
)

bart_tokenizer_local = BartTokenizer.from_pretrained(
    "/home/star/zkx/CLAP/model/bart-base",
    local_files_only=True
)

def tokenizer(text, tmodel="roberta", max_length=77):
    """
    修改后的分词函数，确保引用正确的本地分词器实例
    """
    if tmodel == "transformer":
        return clip_tokenizer(text).squeeze(0)

    elif tmodel == "bert":
        result = bert_tokenizer_local( # 修改点：引用正确的变量
            text,
            padding="max_length",
            truncation=True,
        ...
        result = roberta_tokenizer_local( # 修改点：引用正确的变量
            text,
            padding="max_length",
            truncation=True,
            max_length=max_length,
        ...
        result = bart_tokenizer_local( # 修改点：引用正确的变量
            text,
            padding="max_length",
            truncation=True,
            max_length=max_length,
