...
def add_rank_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["is_base_top2"] = (out["base_rank"] <= 2).astype(int)
    out["is_orig_top2"] = (out["orig_rank"] <= 2).astype(int)
    out["is_ours_top2"] = (out["ours_rank"] <= 2).astype(int)
    return out
...
def add_gap_impact_and_weights(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    impact_vals: List[float] = []
    strong_comp_vals: List[int] = []

    for _, group in out.groupby("sample_id", sort=False):
        gt_row = group[group["is_ground_truth"].astype(int) == 1]
        if gt_row.empty:
            impact_vals.extend([0.0] * len(group))
            strong_comp_vals.extend([0] * len(group))
            continue

        gt_orig = float(gt_row.iloc[0]["orig_score"])
        gt_ours = float(gt_row.iloc[0]["ours_score"])

        for _, row in group.iterrows():
            if int(row["is_ground_truth"]) == 1:
                neg_orig = group[group["is_ground_truth"].astype(int) == 0]["orig_score"]
                neg_ours = group[group["is_ground_truth"].astype(int) == 0]["ours_score"]
                top_neg_orig = float(neg_orig.max()) if not neg_orig.empty else gt_orig
                top_neg_ours = float(neg_ours.max()) if not neg_ours.empty else gt_ours
                gap_orig = float(row["orig_score"]) - top_neg_orig
                gap_ours = float(row["ours_score"]) - top_neg_ours
            else:
                gap_orig = gt_orig - float(row["orig_score"])
                gap_ours = gt_ours - float(row["ours_score"])

            impact_vals.append(abs(gap_orig - gap_ours))
            strong_comp_vals.append(
                int(
                    (int(row["is_ground_truth"]) == 0)
                    and (
                        int(row["base_rank"]) <= 2
                        or int(row["orig_rank"]) <= 2
                        or int(row["ours_rank"]) <= 2
                    )
                )
            )

    out["gap_impact"] = np.array(impact_vals, dtype=float)
    out["is_strong_competitor"] = np.array(strong_comp_vals, dtype=int)

    impact_q75 = float(out["gap_impact"].quantile(0.75)) if len(out) else 0.0
    impact_scale = np.ones(len(out), dtype=float)
    if impact_q75 > 1e-8:
        impact_scale = 1.0 + np.clip(out["gap_impact"].to_numpy() / impact_q75, 0.0, 3.0)

    base_weight = np.ones(len(out), dtype=float)
    ...
    base_weight[gt_orig.to_numpy()] = 5.0
    base_weight[gt_ours.to_numpy()] = 2.0
    strong_comp = out["is_strong_competitor"].astype(int) == 1
    base_weight[strong_comp.to_numpy()] = np.maximum(base_weight[strong_comp.to_numpy()], 2.0)

    out["sample_weight"] = base_weight * impact_scale
    return out
...
def run_lodo_top2_gap_weighted_lr(target_name: str, dataset_specs: List[DatasetSpec], out_dir: str) -> None:
    ...
    train_df = add_gap_impact_and_weights(train_df)
    ...
    target_eval = add_gap_impact_and_weights(target_eval)
    target_eval = target_df.merge(
        target_eval[["sample_id", "candidate_index", "is_router_candidate", "gap_impact", "is_strong_competitor"]],
        on=["sample_id", "candidate_index"],
        how="left",
    )
    target_eval["is_router_candidate"] = target_eval["is_router_candidate"].fillna(0).astype(int)
    target_eval["gap_impact"] = target_eval["gap_impact"].fillna(0.0)
    target_eval["is_strong_competitor"] = target_eval["is_strong_competitor"].fillna(0).astype(int)
    ...
    metrics = {
        ...
        "top2_gap_weighted_lr": _rank_metrics(target_eval, "router_score"),
    }

    diagnostics = {
        "variant_name": "top2_gap_weighted_lr",
        ...
        "sample_weight_mean": float(train_df["sample_weight"].mean()),
        "gap_impact_mean_train": float(train_df["gap_impact"].mean()),
        "gap_impact_gt_mean_train": float(train_df.loc[train_df["is_ground_truth"].astype(int) == 1, "gap_impact"].mean()),
        "gap_impact_non_gt_mean_train": float(train_df.loc[train_df["is_ground_truth"].astype(int) == 0, "gap_impact"].mean()),
    ...
    target_eval.to_csv(os.path.join(out_dir, f"{target_name}_top2_gap_weighted_lr_predictions.csv"), index=False)
    with open(os.path.join(out_dir, "results_top2_gap_weighted_lr.json"), "w", encoding="utf-8") as f:
        json.dump(
            {
    ...
    print("Variant: top2_gap_weighted_lr")
    print()
    print("Method                 Hit@1   Hit@3   Hit@5    MRR")
    print("----------------------------------------------------")
    for name in ["OriginalAgg", "Ours", "OracleCandidate", "top2_gap_weighted_lr"]:
        row = metrics[name]
