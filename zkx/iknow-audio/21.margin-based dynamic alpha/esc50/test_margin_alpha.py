...
OUTPUT_JSON = "/data/zkx/zkx/iknow-audio/21.margin-based dynamic alpha/esc50/results_margin_alpha.json"
...
def collect_margin_quantiles(clap_model, dataset, text_embeds):
    margins = []
    skipped = 0
    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="Collect ESC margins"):
...
def print_final_tables(results):
    ...
    order = ["Baseline", "iKnow", "Full2Hop", "Selective2Hop_StaticAlpha", "Selective2Hop_MarginAlpha"]
    print("\n" + "=" * 160)
    print(f"{'Metric':<8} | {'Baseline':<10} | {'iKnow':<10} | {'Full2Hop':<12} | {'Sel2 StaticAlpha':<18} | {'Sel2 MarginAlpha':<18}")
    print("-" * 160)
...
@torch.no_grad()
def main():
    print("Starting ESC50 margin-based dynamic alpha pilot")
    ...
    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="ESC50 margin alpha"):
        audio_path = sample["audio_path"]
        true_indices = sample["true_indices"]
        if not os.path.exists(audio_path) or not true_indices:
            skipped_count += 1
