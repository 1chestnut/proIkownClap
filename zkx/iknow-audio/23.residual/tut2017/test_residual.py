...
OUTPUT_JSON = "/data/zkx/zkx/iknow-audio/23.residual/tut2017/results_residual.json"
...
def init_results():
    return {
        ...
        "Selective2Hop_PositiveResidual": {
            "ranks": [],
            "times": [],
            "prompts": [],
            "hop2_activation_sample": [],
...
def run_selective2hop_positive_residual(clap_model, audio_embed, cos_sim_orig, top_indices, label_classes, kg_classes, class_labels_set, hop1_relations, hop2_relations, get_tails_fn, prompt_map, q25, q75):
    score = cos_sim_orig.clone()
    prompt_count = 0
    hop2_flags = []
    alpha_values = []
    for ci in top_indices:
        ...
        lambda_i = 1.0 - alpha_i
        score[ci] = cos_sim_orig[ci] + lambda_i * positive_gain
    extras = {
        "hop2_activated": any(hop2_flags),
        "candidate_level_activation_rate": float(np.mean(hop2_flags)) if hop2_flags else 0.0,
...
def print_final_tables(results):
    ...
    order = ["Baseline", "iKnow", "Full2Hop", "Selective2Hop_OriginalAgg", "Selective2Hop_StaticAlpha", "Selective2Hop_PositiveResidual"]
    print("\n" + "=" * 180)
    print(f"{'Metric':<8} | {'Baseline':<10} | {'iKnow':<10} | {'Full2Hop':<12} | {'Sel2 OriginalAgg':<18} | {'Sel2 StaticAlpha':<18} | {'Sel2 PosResidual':<18}")
    print("-" * 180)
...
@torch.no_grad()
def main():
    print("Starting TUT2017 positive-residual fusion pilot")
    print(f"Device: {DEVICE}")
    print("Prompt policy: baseline label-only, iKnow uses pure single-hop concat without Top-P, selective hop1/hop2 use LLM prompts with concat fallback; residual fusion only adds positive KG gain")
    ...
    print(f"Candidate gain quantiles for positive residual: q25={q25:.6f}, q75={q75:.6f}, skipped={gain_skipped}")
    ...
    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="TUT2017 candidate gain"):
        ...
        t = time.time()
        score, prompt_count, extras = run_selective2hop_positive_residual(
        ...
        results["Selective2Hop_PositiveResidual"]["ranks"].append(rank_of_true(score, true_indices))
        results["Selective2Hop_PositiveResidual"]["times"].append(audio_ms + (time.time() - t) * 1000.0)
        results["Selective2Hop_PositiveResidual"]["prompts"].append(prompt_count)
        results["Selective2Hop_PositiveResidual"]["hop2_activation_sample"].append(bool(extras["hop2_activated"]))
        results["Selective2Hop_PositiveResidual"]["hop2_activation_candidate"].append(float(extras["candidate_level_activation_rate"]))
        results["Selective2Hop_PositiveResidual"]["alphas"].append(float(extras["alpha_mean"]))
