...
# 动态 α 范围（仍用于融合，但不输出统计）
ALPHA_MIN = 0.4
ALPHA_MAX = 0.8

# ========== 关系权重配置 (基于 iKnow-audio 图5) ==========
RELATION_WEIGHTS = {
    'is variant of': 1.0,
    'has parent': 1.0,
    'scene contains': 1.0,
    'described by': 0.9,
    'event composed of': 0.9,
}
DEFAULT_REL_WEIGHT = 0.8
...
@torch.no_grad()
def main():
    print("🚀 启动 TUT2017 实验: [多跳图谱] + [LLM] + [动态α] + [关系权重]...")
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="M3+LLM+RelWeight Pipeline"):
        ...
        m3_ranks.append(np.where(sorted_indices_kg == true_idx)[0][0] + 1)

    # ==========================================
    # 4. 打印对比表 (仅 Baseline 和 M3+关系权重)
    # ==========================================
    ...
    print("\n💡 实验说明：已为两跳检索均加入关系权重，权重依据 iKnow-audio 图5 设定。")
    print("若需调整，可修改 RELATION_WEIGHTS 字典中的数值。")
