...
# ========== 关系权重配置 (基于 FSD50K 分类学特性) ==========
RELATION_WEIGHTS = {
    'belongs to class': 1.0,
    'has parent': 1.0,
    'is a type of': 0.8,
}
...
@torch.no_grad()
def main():
    print("🚀 启动 FSD50K 实验: [多跳图谱] + [LLM] + [动态α] + [关系权重]...")
    ...
    for _, row in tqdm(eval_df.iterrows(), total=len(eval_df), desc="M3+LLM+RelWeight Pipeline"):
        fname = str(row['fname'])
    ...
    # ==========================================
    # 4. 打印格式化对比表 (仅 Baseline 和 M3+关系权重)
    # ==========================================
    ...
    print("\n💡 实验说明：已为两跳检索均加入关系权重，权重依据 FSD50K 分类学特性设定。")
