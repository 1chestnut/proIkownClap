...
# 🔥🔥🔥 核心救火修改 🔥🔥🔥
# 把放水的 -0.02 改为严苛的 0.02。
# 在 DCASE17 里，不允许模型随便偷懒！
...
# ==========================================
# 3. 主程序：多跳 + 严苛早退 + 动态α + (Cat + LogSumExp)
# ==========================================
@torch.no_grad()
def main():
    print("🚀 启动 DCASE17 满血救火版: [严苛早退] + [动态α] + [原味 Cat+LogSumExp]...")
    ...
    ranks = {'Baseline': [], 'Ours_StrictAdaptive': []}
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Strict-Adaptive Pipeline"):
        ...
        for c_idx in top_k_indices:
            ...
            # ==========================================
            # STAGE 3: GRASP 剪枝与原汁原味的 LogSumExp 涌现池化
            # ==========================================
        ...
        ranks['Ours_StrictAdaptive'].append(min([np.where(sorted_indices_kg == t_idx)[0][0] + 1 for t_idx in true_indices]))

    # ========== 结果输出 ==========
    b_res = compute_metrics(ranks['Baseline'])
    m3_res = compute_metrics(ranks['Ours_StrictAdaptive'])

    col_w = 28
    header = f"{'Metric':<8} | {'Baseline':<{col_w}} | {'Ours (Strict Adaptive M3)':<{col_w}}"
    print("\n" + "=" * len(header))
    print(header)
    print("-" * len(header))
    ...
    print("\n⚡ [自适应检索效率统计]")
    print(f"动态阈值参数 (Relative Margin) : {RELATIVE_MARGIN} (严苛逼问模式)")
