...
OUTPUT_JSON = "/data/zkx/zkx/iknow-audio/21.margin-based dynamic alpha/tut2017/results_margin_alpha.json"
...
def compute_margin(cos_sim_orig):
    top2 = torch.topk(cos_sim_orig, k=min(2, cos_sim_orig.numel())).values
    if top2.numel() < 2:
        return 0.0
    return float((top2[0] - top2[1]).item())


def margin_alpha(cos_sim_orig, q25, q75):
    if q75 <= q25:
        return (ALPHA_MIN + ALPHA_MAX) / 2.0
    delta = compute_margin(cos_sim_orig)
    u = (delta - q25) / (q75 - q25)
...
@torch.no_grad()
def collect_margin_quantiles(clap_model, dataset, text_embeds):
    margins = []
    skipped = 0
    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="Collect TUT margins"):
        ...
        margins.append(compute_margin(cos_sim_orig))
    q25 = float(np.quantile(margins, 0.25)) if margins else 0.0
    q75 = float(np.quantile(margins, 0.75)) if margins else 1.0
    return q25, q75, skipped


def print_final_tables(results):
    metrics = {name: compute_metrics(results[name]["ranks"]) for name in results}
    order = ["Baseline", "iKnow", "Full2Hop", "Selective2Hop_OriginalAgg", "Selective2Hop_StaticAlpha", "Selective2Hop_MarginAlpha"]
    print("\n" + "=" * 180)
    print(f"{'Metric':<8} | {'Baseline':<10} | {'iKnow':<10} | {'Full2Hop':<12} | {'Sel2 OriginalAgg':<18} | {'Sel2 StaticAlpha':<18} | {'Sel2 MarginAlpha':<18}")
    print("-" * 180)
...
@torch.no_grad()
def main():
    print("Starting TUT2017 margin-based dynamic alpha pilot")
    ...
    q25, q75, margin_skipped = collect_margin_quantiles(clap_model, dataset, text_embeds)
    print(f"Margin quantiles from baseline: q25={q25:.6f}, q75={q75:.6f}, skipped={margin_skipped}")
    results = init_results()
    skipped_count = 0

    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="TUT2017 margin alpha"):
        audio_path = sample["audio_path"]
        true_indices = sample["true_indices"]
        if not os.path.exists(audio_path) or not true_indices:
            skipped_count += 1
        ...
        alpha_margin = margin_alpha(cos_sim_orig, q25, q75)
        ...
        t = time.time()
        score, prompt_count, extras = run_selective2hop(clap_model, audio_embed, cos_sim_orig, top_indices, label_classes, kg_classes, class_labels_set, hop1_relations, hop2_relations, get_tails, prompt_map, alpha_margin)
        results["Selective2Hop_MarginAlpha"]["ranks"].append(rank_of_true(score, true_indices))
        results["Selective2Hop_MarginAlpha"]["times"].append(audio_ms + (time.time() - t) * 1000.0)
        results["Selective2Hop_MarginAlpha"]["prompts"].append(prompt_count)
        results["Selective2Hop_MarginAlpha"]["hop2_activation_sample"].append(bool(extras["hop2_activated"]))
        results["Selective2Hop_MarginAlpha"]["hop2_activation_candidate"].append(float(extras["candidate_level_activation_rate"]))
        results["Selective2Hop_MarginAlpha"]["alphas"].append(alpha_margin)
