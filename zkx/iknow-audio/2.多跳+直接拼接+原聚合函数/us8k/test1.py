...
@torch.no_grad()
def main():
    print(f"🚀 启动 iKnow-audio US8K 权重消融实验...")
    print(f"📊 正在验证 Alpha 梯度: {ALPHA_LIST}")
    ...
    TARGET_RELATIONS = ['overlaps with', 'occurs in', 'associated with environment', 'localized in', 'used for', 'part of scene']
    ...
    print(f"🎵 推理开始...")
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Ablation Study"):
        ...
        for c_idx in top_k_indices:
            ...
            for r1 in VALID_RELATIONS:
                ...
                for t1 in hop1:
                    if t1.lower().strip() != orig_class_name.lower() and t1.lower().strip() not in class_labels_set:
                        raw_multi_hop.add(t1)
                    for r2 in VALID_RELATIONS:
                        hop2 = get_top_m_tails(t1, r2, TOP_M)
                        for t2 in hop2:
                            if t2.lower().strip() != orig_class_name.lower() and t2.lower().strip() not in class_labels_set:
    ...
    col_w = 11
    table_header = f"{'Metric':<8} | {'Baseline':<{col_w}} | {'LME':<{col_w}}"
    for a in ALPHA_LIST:
        table_header += f" | {'M(' + str(a) + ')':<{col_w}}"
