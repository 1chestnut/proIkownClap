...
def _bucket_route_profile(df: pd.DataFrame, feature_col: str, bins: int = 4) -> Dict[str, float]:
    ...
    for bucket, group in key_df.groupby(bucket_ids, observed=True):
        out[str(bucket)] = float(group["router_choose_orig"].mean())
    return out


def _oracle_bucket_profile(df: pd.DataFrame, feature_col: str, bins: int = 4) -> Dict[str, float]:
    key_df = df[df["is_key_candidate"].astype(int) == 1].copy()
    if key_df.empty:
        return {}
    try:
    ...
    out = {}
    for bucket, group in key_df.groupby(bucket_ids, observed=True):
        out[str(bucket)] = float(group["oracle_label"].astype(int).mean())
    return out


def _true_label_profiles(df: pd.DataFrame) -> Dict[str, Dict[str, float]]:
    gt_df = df[df["is_ground_truth"].astype(int) == 1].copy()
    if gt_df.empty:
        return {"oracle_choose_orig": {}, "router_choose_orig": {}}
    oracle = gt_df.groupby("true_label")["oracle_label"].mean().sort_values(ascending=False)
    router = gt_df.groupby("true_label")["router_choose_orig"].mean().sort_values(ascending=False)
    return {
        "oracle_choose_orig": {str(k): float(v) for k, v in oracle.items()},
        "router_choose_orig": {str(k): float(v) for k, v in router.items()},
    }
...
def run_lodo_variant(
    ...
    diagnostics = {
        ...
        "true_candidate_match_accuracy": float(
            np.mean(
                target_eval.loc[gt_mask, "router_choose_orig"].astype(int).to_numpy()
                == target_eval.loc[gt_mask, "oracle_label"].astype(int).to_numpy()
            )
        ),
        "hard_negative_match_accuracy": float(
            np.mean(
                target_eval.loc[key_mask & neg_mask, "router_choose_orig"].astype(int).to_numpy()
                == target_eval.loc[key_mask & neg_mask, "oracle_label"].astype(int).to_numpy()
            )
        ) if np.any(key_mask & neg_mask) else 0.0,
        ...
        **_prob_stats(probs),
        ...
        "router_margin_bucket_profile": _bucket_route_profile(target_eval, "baseline_margin"),
        "oracle_margin_bucket_profile": _oracle_bucket_profile(target_eval, "baseline_margin"),
        "router_entropy_bucket_profile": _bucket_route_profile(target_eval, "entropy"),
        "oracle_entropy_bucket_profile": _oracle_bucket_profile(target_eval, "entropy"),
        "true_label_profiles": _true_label_profiles(target_eval),
    }
