...
OUTPUT_JSON = "/data/zkx/zkx/iknow-audio/27.residual_moe/esc50/results_residual_moe.json"
...
def collect_residual_temperature(clap_model, dataset, text_embeds, label_classes, kg_classes, class_labels_set, hop1_relations, hop2_relations, get_tails, prompt_map):
    deltas = []
    skipped = 0
    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="Collect ESC residual deltas"):
...
def print_final_tables(results):
    ...
    order = ["Baseline", "iKnow", "Full2Hop", "Selective2Hop_StaticAlpha", "Selective2Hop_ResidualMoE"]
    print("\n" + "=" * 160)
    print(f"{'Metric':<8} | {'Baseline':<10} | {'iKnow':<10} | {'Full2Hop':<12} | {'Sel2 StaticAlpha':<18} | {'Sel2 ResidualMoE':<18}")
    print("-" * 160)
...
@torch.no_grad()
def main():
    print("Starting ESC50 residual-MoE fusion pilot")
    print(f"Device: {DEVICE}")
    print("Prompt policy: baseline label-only, iKnow uses pure single-hop concat without Top-P, selective hop1/hop2 use filtered acoustic LLM prompts with concat fallback; final fusion preserves baseline and adds only positive hop1/hop2 residual gains")
    ...
    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="ESC50 residual MoE"):
        audio_path = sample["audio_path"]
        true_indices = sample["true_indices"]
        if not os.path.exists(audio_path) or not true_indices:
            skipped_count += 1
