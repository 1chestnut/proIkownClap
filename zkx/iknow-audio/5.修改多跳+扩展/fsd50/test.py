...
@torch.no_grad()
def main():
    print("🚀 启动 FSD50K 终极版: [多跳图谱] + [LLM自然语义] + [M3衰减聚合]...")
    ...
    for _, row in tqdm(eval_df.iterrows(), total=len(eval_df), desc="M3+LLM Pipeline"):
        fname = str(row['fname'])
    ...
    print("\n💡 终极版机制解析 (FSD50K):")
    print("1. 深度整合：分类学图谱构建广度，LLM赋能场景深度。")
    print("2. 衰减拦截：M3 的 0.85 衰减严密把控了 Taxonomy 分类学向极度宽泛父类滑落的风险！")
