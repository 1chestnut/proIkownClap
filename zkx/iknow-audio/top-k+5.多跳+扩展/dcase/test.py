...
@torch.no_grad()
def main():
    print("🚀 启动 DCASE17 终极版: [自适应动态截断] + [多跳图谱] + [LLM自然语义] + [M3衰减聚合]...")
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Adaptive M3+LLM DCASE"):
        ...
        for c_idx in top_k_indices:
            ...
            # --- 挖掘受限的多跳知识 (换用自适应抽取 get_adaptive_tails) ---
    ...
    print("2. 自适应截断(Adaptive Cutoff) 确保我们只为高潜力类别挖掘高确信度的邻居。")
    print("3. 最后配合 M3 的衰减软池化，成就了既有广度、又有精度的大一统模型！")
