...
OUTPUT_JSON = "/data/zkx/zkx/iknow-audio/24.branch_lse/tut2017/results_branch_lse.json"
...
def branch_lse_fusion(base_score, kg_score, alpha_i):
    branch_inputs = torch.stack([
        base_score * alpha_i * LOGIT_SCALE,
        kg_score * (1.0 - alpha_i) * LOGIT_SCALE,
    ])
    return (torch.logsumexp(branch_inputs, dim=0) - np.log(branch_inputs.numel())) / LOGIT_SCALE


def run_selective2hop_branch_lse(clap_model, audio_embed, cos_sim_orig, top_indices, label_classes, kg_classes, class_labels_set, hop1_relations, hop2_relations, get_tails_fn, prompt_map, q25, q75):
    score = cos_sim_orig.clone()
    prompt_count = 0
    hop2_flags = []
    alpha_values = []
    for ci in top_indices:
        soft_s, used_prompts, hop2_flag = build_selective_kg_score(
        ...
        if soft_s is None:
            ...
            continue
        gain_value = float((soft_s - cos_sim_orig[ci]).item())
        alpha_i = candidate_gain_alpha(gain_value, q25, q75)
        alpha_values.append(alpha_i)
        score[ci] = branch_lse_fusion(cos_sim_orig[ci], soft_s, alpha_i)
    extras = {
        "hop2_activated": any(hop2_flags),
        "candidate_level_activation_rate": float(np.mean(hop2_flags)) if hop2_flags else 0.0,
...
def print_final_tables(results):
    ...
    order = ["Baseline", "iKnow", "Full2Hop", "Selective2Hop_OriginalAgg", "Selective2Hop_StaticAlpha", "Selective2Hop_BranchLSE"]
    print("\n" + "=" * 180)
    print(f"{'Metric':<8} | {'Baseline':<10} | {'iKnow':<10} | {'Full2Hop':<12} | {'Sel2 OriginalAgg':<18} | {'Sel2 StaticAlpha':<18} | {'Sel2 BranchLSE':<18}")
    print("-" * 180)
...
@torch.no_grad()
def main():
    print("Starting TUT2017 branch-LSE fusion pilot")
    print(f"Device: {DEVICE}")
    print("Prompt policy: baseline label-only, iKnow uses pure single-hop concat without Top-P, selective hop1/hop2 use LLM prompts with concat fallback; final fusion uses two-branch LSE")
    ...
    print(f"Candidate gain quantiles for branch-LSE: q25={q25:.6f}, q75={q75:.6f}, skipped={gain_skipped}")
    ...
    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="TUT2017 candidate gain"):
        ...
        t = time.time()
        score, prompt_count, extras = run_selective2hop_branch_lse(
        ...
        results["Selective2Hop_BranchLSE"]["ranks"].append(rank_of_true(score, true_indices))
        results["Selective2Hop_BranchLSE"]["times"].append(audio_ms + (time.time() - t) * 1000.0)
        results["Selective2Hop_BranchLSE"]["prompts"].append(prompt_count)
        results["Selective2Hop_BranchLSE"]["hop2_activation_sample"].append(bool(extras["hop2_activated"]))
        results["Selective2Hop_BranchLSE"]["hop2_activation_candidate"].append(float(extras["candidate_level_activation_rate"]))
        results["Selective2Hop_BranchLSE"]["alphas"].append(float(extras["alpha_mean"]))
