...
# 核心超参数
TOP_K = 15      # 🌟 FSD50K 候选类别截断 (类别多，必须大)
TOP_M = 3       # 知识图谱每跳分支
TOP_P = 5       # GRASP 剪枝保留数
...
@torch.no_grad()
def main():
    print("🚀 启动 FSD50K 终极完全体: [多跳] + [早退] + [动态α] + [🔥KGE软注入]...")
    ...
    for _, row in tqdm(eval_df.iterrows(), total=len(eval_df), desc="Ultimate M3 Pipeline"):
        fname = str(row['fname'])
        ...
        for c_idx in top_k_indices:
            ...
            if max_hop1_score >= tau_dynamic:
                ...
                if len(candidate_info_hop2) > 0:
                    ...
                    # 拼接：一跳乘 1.0，二跳乘 物理衰减 DECAY_GAMMA
