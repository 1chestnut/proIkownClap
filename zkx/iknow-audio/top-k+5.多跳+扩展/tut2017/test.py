...
@torch.no_grad()
def main():
    print("🚀 启动 TUT2017 终极版: [自适应动态截断] + [多跳图谱] + [LLM自然语义] + [M3衰减聚合]...")
    ...
    # 💡 结构层修复：TUT是场景识别，极易发生“场景->包含物体->其他场景”的漂移
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Adaptive M3+LLM Pipeline"):
        ...
        for c_idx in top_k_indices:
            ...
            for r1 in HOP1_RELATIONS:
                for t1 in get_adaptive_tails(kg_scene_name, r1):
                    t1_c = t1.lower().strip()
            ...
            if len(candidate_info) > 0:
                ...
                # 🌟 修复机制：逐句提取特征，完美规避 LLM 长短不一导致的 stack 报错
                p_embs_list = []
                for p in prompts:
    ...
    print("\n💡 终极版机制解析 (TUT2017):")
    print("1. 动态截断: 场景区分度高时果断少查，场景模糊时大胆多查，绝不错杀！")
    print("2. 衰减限制: 通过二跳解绑 'scene contains' 且引入 0.85 衰减，严密防范环境音的连锁漂移。")
    print("3. LLM 润色: 把抽象的图谱结构变成了包含时间、空间的自然听觉画面！")
