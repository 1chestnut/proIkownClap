...
# ==========================================
# 3. 主程序：自适应截断 + 多跳 + LLM + M3
# ==========================================
@torch.no_grad()
def main():
    print("🚀 启动 US8K 终极版: [自适应截断] + [多跳图谱] + [LLM自然语义] + [M3衰减聚合]...")
    ...
    def get_adaptive_tails(head_entity, relation):
        ...
        try:
            ...
            # 动态截断：保留分数在 (best_score - MARGIN_M) 以内的实体，最多 MAX_M 个
    ...
    print("\n💡 自适应版机制解析:")
    print("1. Adaptive K: 音频极其清晰时不再拉废柴类做图谱补充，节省算力且防止底噪反超。")
    print("2. Adaptive M: 图谱关联微弱时坚决只取最高分实体，不为凑够数量而引入偏题知识。")
    print("3. LLM + M3: 用最生动的语言描述声音，用最严谨的数学规律 (0.85 Decay) 守护准确性！")
