...
LLM_PROMPTS_PATH = "/home/star/zkx/iknow-audio/单跳+提示词扩展+0.85权重/us8k/llm_prompts.json"
...
def load_llm_prompts(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        ...
        for key, text in data.items():
            parts = key.split("||")
            if len(parts) == 3:
                # 统一键值格式：head||relation||tail (均为小写)
...
@torch.no_grad()
def main():
    print("🚀 启动 US8K 单跳+LLM文本扩充 全权重对齐实验...")
