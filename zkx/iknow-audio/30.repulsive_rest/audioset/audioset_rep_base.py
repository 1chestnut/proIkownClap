...
OUTPUT_JSON = "/data/zkx/zkx/iknow-audio/17.消融/audioset/results_ablation.json"
PROGRESS_JSON = "/data/zkx/zkx/iknow-audio/17.消融/audioset/progress.json"
...
HOP1_RELATIONS = ["belongs to class", "has parent", "is a type of", "has children"]
HOP2_RELATIONS = ["belongs to class", "has parent", "is a type of", "has children"]
...
def iter_samples(dataset):
    mid_to_name = dataset["mid_to_name"]
    class_to_idx = dataset["class_to_idx"]
    for _, row in dataset["df"].iterrows():
        translated = []
        clean_str = str(row["labels"]).replace('"', "").strip()
        for part in [p.strip() for p in clean_str.split(",")]:
            if part in mid_to_name:
                translated.append(mid_to_name[part])
            elif part in class_to_idx:
                translated.append(part)
        true_indices = [class_to_idx[label] for label in translated if label in class_to_idx]
        yield {"audio_path": os.path.join(AUDIO_DIR, row["audio_file"]), "true_indices": true_indices}


@torch.no_grad()
def main():
    print("Starting AudioSet ablation: Baseline / iKnow / Full2Hop / Ours / Selective2Hop_OriginalAgg / Selective2Hop_NoLLM")
    ...
    for sample_idx in tqdm(range(start_idx, total_samples), total=total_samples, initial=start_idx, desc="AudioSet ablation"):
