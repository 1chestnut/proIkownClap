...
OUTPUT_JSON = "/data/zkx/zkx/iknow-audio/25.gmu_residual/esc50/results_gmu_residual.json"
...
def collect_gain_quantiles(clap_model, dataset, text_embeds, label_classes, kg_classes, class_labels_set, hop1_relations, hop2_relations, get_tails, prompt_map):
    gains = []
    skipped = 0
    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="Collect ESC candidate gains"):
...
def print_final_tables(results):
    ...
    order = ["Baseline", "iKnow", "Full2Hop", "Selective2Hop_StaticAlpha", "Selective2Hop_GMUResidual"]
    print("\n" + "=" * 160)
    print(f"{'Metric':<8} | {'Baseline':<10} | {'iKnow':<10} | {'Full2Hop':<12} | {'Sel2 StaticAlpha':<18} | {'Sel2 GMUResidual':<18}")
    print("-" * 160)
...
@torch.no_grad()
def main():
    print("Starting ESC50 GMU-style residual fusion pilot")
    print(f"Device: {DEVICE}")
    print("Prompt policy: baseline label-only, iKnow uses pure single-hop concat without Top-P, selective hop1/hop2 use filtered acoustic LLM prompts with concat fallback; final fusion uses GMU-style gated positive residual over OriginalAgg")
    ...
    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="ESC50 candidate gain"):
        audio_path = sample["audio_path"]
        true_indices = sample["true_indices"]
        if not os.path.exists(audio_path) or not true_indices:
            skipped_count += 1
