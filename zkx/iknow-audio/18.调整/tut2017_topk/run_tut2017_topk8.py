...
OUTPUT_DIR = "/data/zkx/zkx/iknow-audio/18.\u8c03\u6574/tut2017_topk"
...
def load_module(path):
    spec = importlib.util.spec_from_file_location("tut_ablation_module_topk", path)
...
def init_progress(total):
    return {"next_index": 0, "total_samples": total, "results": {"Ours_TopK8": {"ranks": [], "times": [], "prompts": [], "hop2_activation_sample": [], "hop2_activation_candidate": []}}}
...
def load_progress(total):
    ...
    old = payload.get("results", {}).get("Ours_TopK8", {})
    for k in fresh["results"]["Ours_TopK8"]:
        fresh["results"]["Ours_TopK8"][k] = old.get(k, [])
    return fresh
...
def print_compare(results):
    ...
    print("\n" + "=" * 132)
    print(f"{'TUT2017 TopK Diagnostic':<24} | {'OriginalAgg':<12} | {'Current_Ours':<14} | {'Ours_TopK8':<14}")
    print("-" * 132)
    for idx, name in enumerate(["Hit@1", "Hit@3", "Hit@5", "MRR"]):
        print(f"{name:<24} | {ref_orig[name]:<12.2f} | {ref_ours[name]:<14.2f} | {m[idx]:<14.2f}")
    print("\n" + "=" * 132)
    print(f"Ours_TopK8 stats: prompts={s['avg_prompts']:.1f}, time={s['avg_time_ms']:.1f} ms, activation(sample)={s['hop2_activation_sample']*100:.1f}%, activation(candidate)={s['hop2_activation_candidate']*100:.1f}%, top_k={NEW_TOP_K}")


@ab.torch.no_grad()
def main():
    print(f"Starting TUT2017 diagnostic: top-k expanded to {NEW_TOP_K}")
    ...
    for idx in ab.tqdm(range(start_index, len(samples)), desc="TUT2017 topk8"):
        sample = samples[idx]
