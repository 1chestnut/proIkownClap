...
TOP_K_CLAP = 10
TOP_M_PER_REL = 1
RELATIONS_Q = ['caused by', 'co-occurs with', 'associated with environment']

# 聚合方式选择
AGG_MODE = 'max'          # 可选: 'max', 'logsumexp'
TEMPERATURE = 20.0        # 仅用于 logsumexp
        ...
        if mode == 'baseline':
            ...
            for idx, (label, orig_score) in enumerate(zip(top_k_labels, top_k_scores)):
                ...
                if AGG_MODE == 'max':
                    ...
                    # LogSumExp 含温度缩放
                    enhanced_scores.append(orig_score)
                    scores_tensor = torch.tensor(enhanced_scores, device=DEVICE) * TEMPERATURE
...
def main():
    ...
    print("\n" + "="*60)
    print("📊 UrbanSound8K 零样本分类结果 (10折平均)")
