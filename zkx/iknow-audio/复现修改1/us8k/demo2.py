...
@torch.no_grad()
def main():
    print("🚀 启动 iKnow-audio US8K (MS-CLAP 离线版 + 完美修正 Eq 4)...")
    ...
    # 彻底弃用 "This is a sound of"，直接使用纯标签
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Inference"):
        # US8K 专属路径拼接：audio/foldX/filename.wav
        ...
        # 3. 提取与聚合知识 (绝对尊崇 Equation 4 + LogMeanExp 修正)
