...
LLM_PROMPTS_PATH = "/home/star/zkx/iknow-audio/单跳+提示词扩展+0.85权重/fsd50k/llm_prompts.json"
...
@torch.no_grad()
def main():
    print("🚀 启动 FSD50K 单跳+LLM文本扩充 全权重对齐实验...")
    ...
    print(f"🎵 推理开始 (FSD50K)...")
    for _, row in tqdm(eval_df.iterrows(), total=len(eval_df), desc="Weights Search"):
        fname = str(row['fname'])
        ...
        for c_idx in top_k_indices:
            ...
            if len(unique_prompts) > 0:
                ...
                # ==== 轨道 B: Max(Alpha) 梯度计算 ====
                best_p_score = torch.max(p_scores) # 统一 Max Pooling
                orig_s = cos_sim_orig[c_idx]
                for a in ALPHA_LIST:
                    score_alpha_map[a][c_idx] = (a * orig_s) + ((1.0 - a) * best_p_score)

        # 3. 统计本样本在各权重下的最优名次 (FSD50K 多标签逻辑)
