...
# ==========================================
# 3. 主程序：多跳 + LLM自然语义 + 动态α融合 + 自适应早退 (严格物理隔离)
# ==========================================
@torch.no_grad()
def main():
    print("🚀 启动 TUT2017 终极物理隔离消融实验 (Top-P 降噪修正 & 防崩溃版)...")
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="TUT2017 Evaluation"):
        ...
        for c_idx in top_k_indices:
            ...
            for r in HOP1_RELATIONS:
                for t in get_tails(kg_ent, r):
                    t_c = t.lower().strip()
                    if t_c != orig_c.lower() and t_c not in class_labels_set and t_c != kg_ent.lower(): 
                        tails_h1.add(t)
        ...
        for c_idx in top_k_indices:
            ...
            for r1 in HOP1_RELATIONS:
                for t1 in get_tails(kg_ent, r1):
                    t1_l = t1.lower().strip()
                    if t1_l != orig_c.lower() and t1_l not in class_labels_set and t1_l != kg_ent.lower():
            ...
            for t1_l in h1_map.keys():
                for r2 in HOP2_RELATIONS:
                    for t2 in get_tails(t1_l, r2):
                        t2_l = t2.lower().strip()
                        if t2_l != orig_c.lower() and t2_l not in class_labels_set and t2_l != kg_ent.lower() and t2_l not in h1_map:
        ...
        for c_idx in top_k_indices:
            ...
            for r in HOP1_RELATIONS:
                for t in get_tails(kg_ent, r):
                    t_l = t.lower().strip()
                    if t_l != orig_c.lower() and t_l not in class_labels_set and t_l != kg_ent.lower():
            ...
            if max_h1 >= tau:
                ...
                for t1_l in h1_map_s.keys():
                    for r2 in HOP2_RELATIONS:
                        for t2 in get_tails(t1_l, r2):
                            t2_l = t2.lower().strip()
                            if t2_l != orig_c.lower() and t2_l not in class_labels_set and t2_l != kg_ent.lower() and t2_l not in h1_map_s:
