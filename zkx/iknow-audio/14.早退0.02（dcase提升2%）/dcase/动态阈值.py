...
# ==========================================
# 3. 主程序：多跳 + 全自动动态早退 + 动态α + (Cat + LogSumExp)
# ==========================================
@torch.no_grad()
def main():
    print("🚀 启动 DCASE17 终极进化版: [全自动感知早退] + [动态α] + [原味 Cat+LogSumExp]...")
    ...
    ranks = {'Baseline': [], 'Ours_ConfidenceAware': []}
    
    # 效率与监控统计
    total_candidates_processed = 0
    hop2_triggered_count = 0
    recorded_margins = []  # 记录每一段音频的动态阈值

    print(f"🎵 推理开始 (总计 {len(df)} 个多标签样本)...")
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Confidence-Aware Pipeline"):
        ...
        # 🔥🔥🔥 终极进化：计算 Confidence Gap 并动态生成 Margin 🔥🔥🔥
        top1_score = cos_sim_orig[sorted_indices_baseline[0]].item()
        top2_score = cos_sim_orig[sorted_indices_baseline[1]].item()
        gap = top1_score - top2_score
        
        # 动态映射：差距越小，门槛越严(正数)；差距越大，门槛越松(负数)
        # 钳制在 [-0.03, 0.03] 之间。
        dynamic_margin = max(-0.03, min(0.03, 0.03 - 0.5 * gap))
        ...
        for c_idx in top_k_indices:
            ...
            # 🌟 现在的早退阈值是完全自动感知的！
            tau_dynamic = baseline_score + dynamic_margin
            ...
            # ==========================================
            # STAGE 3: GRASP 剪枝与 LogSumExp 涌现池化
            # ==========================================
        ...
        ranks['Ours_ConfidenceAware'].append(min([np.where(sorted_indices_kg == t_idx)[0][0] + 1 for t_idx in true_indices]))

    # ========== 结果输出 ==========
    b_res = compute_metrics(ranks['Baseline'])
    m3_res = compute_metrics(ranks['Ours_ConfidenceAware'])

    col_w = 28
    header = f"{'Metric':<8} | {'Baseline':<{col_w}} | {'Ours (Confidence M3)':<{col_w}}"
    print("\n" + "=" * len(header))
    print(header)
    print("-" * len(header))
    ...
    avg_margin = np.mean(recorded_margins)
    
    print("\n⚡ [全自动感知检索效率统计]")
    print(f"平均动态阈值 (Avg Dynamic Margin) : {avg_margin:.4f} (模型自适应生成的门槛)")
    print(f"二跳检索触发率 (Hop-2 Trigger Rate) : {hop2_rate:.1f}%")
    print(f"计算节省量 (Computational Savings) : 成功拦截了 {100 - hop2_rate:.1f}% 的冗余深层检索！")
    print("💡 结论: 模型已具备自我感知能力，能根据 Top-1 和 Top-2 的分数差距，全自动调节严苛或宽松的早退门槛。")
