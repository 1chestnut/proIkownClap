...
def select_router_candidates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, group in df.groupby("sample_id", sort=False):
        ...
        idx.update(group[group["base_rank"] <= 2].index.tolist())
        idx.update(group[group["orig_rank"] <= 2].index.tolist())
        idx.update(group[group["ours_rank"] <= 2].index.tolist())
...
def add_rf_weights(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    strong_comp = (
...
def build_sample_gate_thresholds(train_df: pd.DataFrame) -> Dict[str, float]:
    sample_rows = (
        train_df[
            [
                "sample_id",
    ...
    stats = []
    for sample_id, group in sample_rows.groupby("sample_id", sort=False):
        gt_orig = group.loc[group["is_ground_truth"].astype(int) == 1, "orig_score"].tolist()
        gt_ours = group.loc[group["is_ground_truth"].astype(int) == 1, "ours_score"].tolist()
        neg_orig = group.loc[group["is_ground_truth"].astype(int) == 0, "orig_score"]
        neg_ours = group.loc[group["is_ground_truth"].astype(int) == 0, "ours_score"]
        if not gt_orig or neg_orig.empty or neg_ours.empty:
            continue
        gap_orig = float(gt_orig[0] - neg_orig.max())
        gap_ours = float(gt_ours[0] - neg_ours.max())
        sample_oracle_pref = int(gap_orig > gap_ours)
        stats.append(
            {
                "sample_id": sample_id,
                "baseline_margin": float(group.iloc[0]["baseline_margin"]),
                "entropy": float(group.iloc[0]["entropy"]),
                "sample_oracle_pref": sample_oracle_pref,
            }
        )
    sample_df = pd.DataFrame(stats)
    pref_df = sample_df[sample_df["sample_oracle_pref"] == 1]
    if pref_df.empty:
        pref_df = sample_df
    if pref_df.empty:
        return {"margin_tau": 0.05, "entropy_tau": 0.999}
    margin_tau = float(pref_df["baseline_margin"].quantile(0.75))
    entropy_tau = float(pref_df["entropy"].quantile(0.25))
    return {"margin_tau": margin_tau, "entropy_tau": entropy_tau}


def mark_risky_samples(df: pd.DataFrame, thresholds: Dict[str, float]) -> pd.DataFrame:
    out = df.copy()
    risky_mask = (
        (out["baseline_margin"] <= thresholds["margin_tau"])
        | (out["entropy"] >= thresholds["entropy_tau"])
    )
    out["is_risky_sample"] = risky_mask.astype(int)
    return out
...
def prepare_target_table(df: pd.DataFrame, probs: np.ndarray) -> pd.DataFrame:
    ...
    use_orig = (
        (out["is_risky_sample"].astype(int) == 1)
        & (out["is_router_candidate"].astype(int) == 1)
        & (out["router_prob"] > 0.5)
    )
...
def run_lodo_gate_rf(target_name: str, dataset_specs: List[DatasetSpec], out_dir: str) -> None:
    ...
    gate_thresholds = build_sample_gate_thresholds(train_df_full)
    train_df_full = mark_risky_samples(train_df_full, gate_thresholds)
    train_df_full = train_df_full[train_df_full["is_risky_sample"].astype(int) == 1].copy()

    train_df = select_router_candidates(train_df_full)
    train_df = add_rf_weights(train_df)
    ...
    clf.fit(X_train, y_train, sample_weight=train_df["sample_weight"].to_numpy())

    target_eval = target_df.dropna(subset=FEATURE_COLUMNS).copy()
    target_eval = mark_risky_samples(target_eval, gate_thresholds)
    target_router = select_router_candidates(target_eval[target_eval["is_risky_sample"].astype(int) == 1].copy())

    target_eval = target_eval.merge(
        target_router[["sample_id", "candidate_index", "is_router_candidate"]],
    ...
    metrics = {
        ...
        "gate_rf": _rank_metrics(target_eval, "router_score"),
    }

    diagnostics = {
        "variant_name": "gate_rf",
        "gate_thresholds": gate_thresholds,
        "train_rows_before": int(len(pd.concat([df for name, df in tables.items() if name != target_name], ignore_index=True))),
        "train_rows_after_gate": int(len(train_df_full)),
        "train_rows_after_router_select": int(len(train_df)),
        "train_choose_orig_ratio": float(np.mean(y_train)) if len(y_train) else 0.0,
        "sample_weight_mean": float(train_df["sample_weight"].mean()) if len(train_df) else 0.0,
        ...
        "risky_sample_ratio_test": float(target_eval[["sample_id", "is_risky_sample"]].drop_duplicates()["is_risky_sample"].mean()),
        ...
        **_prob_stats(probs),
        "feature_importance_mean": _safe_float_dict(clf.feature_importances_, FEATURE_COLUMNS),
    ...
    target_eval.to_csv(os.path.join(out_dir, f"{target_name}_gate_rf_predictions.csv"), index=False)
    with open(os.path.join(out_dir, "results_gate_rf.json"), "w", encoding="utf-8") as f:
        json.dump(
            {
    ...
    print("Variant: gate_rf")
    print()
    print("Method                 Hit@1   Hit@3   Hit@5    MRR")
    print("----------------------------------------------------")
    for name in ["OriginalAgg", "Ours", "OracleCandidate", "gate_rf"]:
        row = metrics[name]
