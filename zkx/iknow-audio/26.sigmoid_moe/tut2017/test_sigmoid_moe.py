...
OUTPUT_JSON = "/data/zkx/zkx/iknow-audio/26.sigmoid_moe/tut2017/results_sigmoid_moe.json"
...
HOP2_RELATIONS = ["is variant of", "has parent", "described by", "event composed of"]
...
def run_selective2hop_originalagg(clap_model, audio_embed, cos_sim_orig, top_indices, label_classes, kg_classes, class_labels_set, hop1_relations, hop2_relations, get_tails_fn, prompt_map):
    score = cos_sim_orig.clone()
    prompt_count = 0
    hop2_flags = []
    for ci in top_indices:
        ...
        if s1.numel() > 0 and max_h1 >= tau:
            ...
            best_scores = safe_topk(s1, TOP_P)
            if best_scores.numel() > 0:
                lse_inputs = torch.cat([cos_sim_orig[ci].unsqueeze(0) * LOGIT_SCALE, best_scores * LOGIT_SCALE])
                score[ci] = (torch.logsumexp(lse_inputs, dim=0) - np.log(lse_inputs.numel())) / LOGIT_SCALE
            prompt_count += len(h1_map)
            continue
        hop2_flags.append(True)
        h2_prompts = build_h2_prompts(h1_map, class_name, class_labels_set, hop2_relations, get_tails_fn, prompt_map)
...
def sigmoid_moe_fusion(base_score, hop1_score, hop2_score, temperature):
    temp = max(float(temperature), 1e-6)
    base_value = float(base_score.item())
    hop1_value = float(hop1_score.item()) if hop1_score is not None else None
    hop2_value = float(hop2_score.item()) if hop2_score is not None else None
    reference = max([value for value in [hop1_value, hop2_value] if value is not None], default=base_value)
    g_base = 1.0 / (1.0 + np.exp(-(base_value - reference) / temp))
    g_h1 = 0.0 if hop1_value is None else 1.0 / (1.0 + np.exp(-((hop1_value - base_value) / temp)))
    h2_ref = max(base_value, hop1_value if hop1_value is not None else base_value)
    g_h2 = 0.0 if hop2_value is None else 1.0 / (1.0 + np.exp(-((hop2_value - h2_ref) / temp)))
    total = g_base + g_h1 + g_h2
    if total <= 1e-8:
        return base_score, (1.0, 0.0, 0.0)
    pi_base = g_base / total
    pi_h1 = g_h1 / total
    pi_h2 = g_h2 / total
    fused = base_score * pi_base
    if hop1_score is not None:
        fused = fused + hop1_score * pi_h1
    if hop2_score is not None:
        fused = fused + hop2_score * pi_h2
    return fused, (pi_base, pi_h1, pi_h2)


def run_selective2hop_sigmoid_moe(clap_model, audio_embed, cos_sim_orig, top_indices, label_classes, kg_classes, class_labels_set, hop1_relations, hop2_relations, get_tails_fn, prompt_map, temperature):
    score = cos_sim_orig.clone()
    prompt_count = 0
    hop2_flags = []
    knowledge_weights = []
    for ci in top_indices:
        hop1_s, hop2_s, used_prompts, hop2_flag = build_selective_branch_scores(
        ...
        fused_score, gate_values = sigmoid_moe_fusion(cos_sim_orig[ci], hop1_s, hop2_s, temperature)
        score[ci] = fused_score
        knowledge_weights.append(float(gate_values[1] + gate_values[2]))
    extras = {
        "hop2_activated": any(hop2_flags),
        "candidate_level_activation_rate": float(np.mean(hop2_flags)) if hop2_flags else 0.0,
        "alpha_mean": float(np.mean(knowledge_weights)) if knowledge_weights else 0.0,
    }
    return score, prompt_count, extras
...
@torch.no_grad()
def collect_moe_temperature(clap_model, dataset, text_embeds, label_classes, kg_classes, class_labels_set, hop1_relations, hop2_relations, get_tails, prompt_map):
    deltas = []
    skipped = 0
    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="Collect TUT branch deltas"):
        ...
        for ci in top_indices:
            hop1_s, hop2_s, _, _ = build_selective_branch_scores(
                clap_model, audio_embed, cos_sim_orig, ci, label_classes, kg_classes,
                class_labels_set, hop1_relations, hop2_relations, get_tails, prompt_map
            )
            if hop1_s is not None:
                deltas.append(abs(float((hop1_s - cos_sim_orig[ci]).item())))
            if hop2_s is not None:
                deltas.append(abs(float((hop2_s - cos_sim_orig[ci]).item())))
    temperature = float(np.quantile(deltas, 0.75)) if deltas else 0.02
    return max(temperature, 1e-3), skipped


def print_final_tables(results):
    metrics = {name: compute_metrics(results[name]["ranks"]) for name in results}
    order = ["Baseline", "iKnow", "Full2Hop", "Selective2Hop_OriginalAgg", "Selective2Hop_StaticAlpha", "Selective2Hop_SigmoidMoE"]
    print("\n" + "=" * 180)
    print(f"{'Metric':<8} | {'Baseline':<10} | {'iKnow':<10} | {'Full2Hop':<12} | {'Sel2 OriginalAgg':<18} | {'Sel2 StaticAlpha':<18} | {'Sel2 SigmoidMoE':<18}")
    print("-" * 180)
...
@torch.no_grad()
def main():
    print("Starting TUT2017 sigmoid-gated MoE fusion pilot")
    print(f"Device: {DEVICE}")
    print("Prompt policy: baseline label-only, iKnow uses pure single-hop concat without Top-P, selective hop1/hop2 use LLM prompts with concat fallback; final fusion uses sigmoid-gated MoE over baseline, hop1, and hop2")
    ...
    moe_temperature, gain_skipped = collect_moe_temperature(
        clap_model, dataset, text_embeds, label_classes, kg_classes,
        class_labels_set, hop1_relations, hop2_relations, get_tails, prompt_map
    )
    print(f"Sigmoid-MoE temperature: temp={moe_temperature:.6f}, skipped={gain_skipped}")
    results = init_results()
    skipped_count = 0

    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="TUT2017 sigmoid MoE"):
        audio_path = sample["audio_path"]
        true_indices = sample["true_indices"]
        if not os.path.exists(audio_path) or not true_indices:
            skipped_count += 1
        ...
        t = time.time()
        score, prompt_count, extras = run_selective2hop_originalagg(clap_model, audio_embed, cos_sim_orig, top_indices, label_classes, kg_classes, class_labels_set, hop1_relations, hop2_relations, get_tails, prompt_map)
        ...
        t = time.time()
        score, prompt_count, extras = run_selective2hop_sigmoid_moe(
            clap_model, audio_embed, cos_sim_orig, top_indices, label_classes, kg_classes,
            class_labels_set, hop1_relations, hop2_relations, get_tails, prompt_map, moe_temperature
        )
        results["Selective2Hop_SigmoidMoE"]["ranks"].append(rank_of_true(score, true_indices))
        results["Selective2Hop_SigmoidMoE"]["times"].append(audio_ms + (time.time() - t) * 1000.0)
        results["Selective2Hop_SigmoidMoE"]["prompts"].append(prompt_count)
        results["Selective2Hop_SigmoidMoE"]["hop2_activation_sample"].append(bool(extras["hop2_activated"]))
        results["Selective2Hop_SigmoidMoE"]["hop2_activation_candidate"].append(float(extras["candidate_level_activation_rate"]))
        results["Selective2Hop_SigmoidMoE"]["alphas"].append(float(extras["alpha_mean"]))
