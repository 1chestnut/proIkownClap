...
@torch.no_grad()
def main():
    print("🚀 启动 TUT2017 双向自适应版: [🔥自适应广度] + [🔥自适应深度/早退] + [动态α]...")
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Dual-Adaptive Pipeline"):
        ...
        ranks['Ours_DualAdaptive'].append(np.where(sorted_indices_kg == true_idx)[0][0] + 1)
    ...
    print(f"1. 广度截断节省算力 (Width Saved) : 成功踢掉了 {width_saved_rate:.1f}% 的无关场景类别！")
    print(f"2. 深度早退节省算力 (Depth Saved) : 成功拦截了 {100 - hop2_rate:.1f}% 的冗余二跳场景发散！")
    print("💡 结论: 面对最容易发生语义漂移的宏观声学场景，模型成功阻断了发散，做到了见好就收。")
