...
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # 🌟 绑定 GPU，根据你的环境调整
...
# 🌟 使用你最新生成的大模型声学扩展 JSON
LLM_PROMPTS_PATH = "/data/zkx/zkx/iknow-audio/9.新Prompt+6./promps/tut2017_llm_prompts.json"
...
@torch.no_grad()
def main():
    print("🚀 启动 TUT2017 终极纯净版: [Baseline] vs [M3-RAG + LLM声学增强]")
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="M3+LLM Pipeline"):
        ...
        # 记录 Ours 最终排名
        m3_llm_ranks.append(np.where(torch.argsort(final_scores, descending=True).cpu().numpy() == true_idx)[0][0] + 1)
