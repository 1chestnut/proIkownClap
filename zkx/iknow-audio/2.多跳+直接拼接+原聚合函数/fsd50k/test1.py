...
@torch.no_grad()
def main():
    print(f"🚀 启动 iKnow-audio FSD50K 权重消融实验...")
    print(f"📊 正在对比: Baseline, LME, 以及 Alpha 梯度 {ALPHA_LIST}")
    ...
    VALID_RELATIONS = [r for r in ['belongs to class', 'has parent', 'is a type of'] if r in training_factory.relation_to_id]
    ...
    for _, row in tqdm(eval_df.iterrows(), total=len(eval_df)):
        ...
        for c_idx in top_k_indices:
            ...
            if len(multi_hop_list) > 0:
                # GRASP 剪枝
                t_embs = to_tensor(clap_model.get_text_embeddings(multi_hop_list)).to(DEVICE).float()
                ...
                p_embs = to_tensor(clap_model.get_text_embeddings(enriched_prompts)).to(DEVICE).float()
        ...
        for a in ALPHA_LIST:
            ...
            alpha_ranks_dict[a].append(min([np.where(r_alpha == t_idx)[0][0] + 1 for t_idx in true_indices]))
    ...
    for i, m_name in enumerate(metrics):
        row_str = f"{m_name:<8} | {b_res[i]:<{col_w}.2f} | {l_res[i]:<{col_w}.2f}"
        for a in ALPHA_LIST:
            a_res = compute_metrics(alpha_ranks_dict[a])
            row_str += f" | {a_res[i]:<{col_w}.2f}"
        print(row_str)
    print("=" * len(header))
