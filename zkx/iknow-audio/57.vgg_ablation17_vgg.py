...
VGG_DIR = "/data/zkx/zkx/iknow-audio/data/vgg"
...
LLM_PROMPTS_PATH = "/data/zkx/zkx/iknow-audio/1.单跳+提示词扩展+0.85权重/vgg/llm_prompts.json"
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "/data/zkx/zkx/iknow-audio/57.vgg_ablation17")
...
def get_kg_entity(class_name):
    return normalize_text(class_name)


def resolve_head_query(head, training_factory):
    if head in training_factory.entity_to_id:
        return head
    lowered = head.lower()
    if lowered in training_factory.entity_to_id:
        return lowered
    return head
...
def load_dataset():
    ...
    if not shard_paths:
        raise FileNotFoundError(f"No test parquet shards found in {VGG_DIR}")
    ...
    for shard_idx, shard_path in enumerate(shard_paths):
        ...
        for row_idx, cap in enumerate(captions):
            label_values.add(cap)
            sample_meta.append({"shard_idx": shard_idx, "row_idx": row_idx, "label": cap})

    label_classes = sorted(label_values)
...
def load_shard_cache(shard_path):
    table = pq.read_table(shard_path, columns=["caption", "audio"])
    data = table.to_pydict()
    return data


def write_audio_bytes(audio_dict, sample_idx):
    os.makedirs(TMP_AUDIO_DIR, exist_ok=True)
    base_name = audio_dict.get("path") or f"sample_{sample_idx:06d}.wav"
    out_path = os.path.join(TMP_AUDIO_DIR, f"{sample_idx:06d}_{os.path.basename(base_name)}")
...
@torch.no_grad()
def main():
    print("Starting VGGSound test ablation: Baseline / iKnow / Full2Hop / Ours / Selective2Hop_OriginalAgg / Selective2Hop_NoLLM")
    print(f"Device: {DEVICE}")
    print("Input: parquet test shards with caption used as class label.")
    ...
    print(f"Unique labels: {len(label_classes)}")
    ...
    for sample_idx in tqdm(range(start_idx, total_samples), total=total_samples, initial=start_idx, desc="VGGSound test ablation"):
        ...
        if current_shard_idx != shard_idx:
            current_cache = load_shard_cache(shard_paths[shard_idx])
            current_shard_idx = shard_idx

        audio_item = current_cache["audio"][row_idx]
        try:
            audio_path = write_audio_bytes(audio_item, sample_idx)
        except Exception:
            skipped_count += 1
