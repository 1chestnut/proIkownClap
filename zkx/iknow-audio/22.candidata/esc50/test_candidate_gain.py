...
OUTPUT_JSON = "/data/zkx/zkx/iknow-audio/22.candidata/esc50/results_candidate_gain.json"
...
def print_final_tables(results):
    ...
    order = ["Baseline", "iKnow", "Full2Hop", "Selective2Hop_StaticAlpha", "Selective2Hop_CandidateGainAlpha"]
    print("\n" + "=" * 160)
    print(f"{'Metric':<8} | {'Baseline':<10} | {'iKnow':<10} | {'Full2Hop':<12} | {'Sel2 StaticAlpha':<18} | {'Sel2 CandidateGain':<18}")
    print("-" * 160)
...
@torch.no_grad()
def main():
    print("Starting ESC50 candidate-gain dynamic alpha pilot")
    print(f"Device: {DEVICE}")
    print("Prompt policy: baseline label-only, iKnow uses pure single-hop concat without Top-P, selective hop1/hop2 use filtered acoustic LLM prompts with concat fallback")
