...
OUTPUT_JSON = "/data/zkx/zkx/iknow-audio/26.sigmoid_moe/esc50/results_sigmoid_moe.json"
...
def collect_moe_temperature(clap_model, dataset, text_embeds, label_classes, kg_classes, class_labels_set, hop1_relations, hop2_relations, get_tails, prompt_map):
    deltas = []
    skipped = 0
    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="Collect ESC branch deltas"):
...
def print_final_tables(results):
    ...
    order = ["Baseline", "iKnow", "Full2Hop", "Selective2Hop_StaticAlpha", "Selective2Hop_SigmoidMoE"]
    print("\n" + "=" * 160)
    print(f"{'Metric':<8} | {'Baseline':<10} | {'iKnow':<10} | {'Full2Hop':<12} | {'Sel2 StaticAlpha':<18} | {'Sel2 SigmoidMoE':<18}")
    print("-" * 160)
...
@torch.no_grad()
def main():
    print("Starting ESC50 sigmoid-gated MoE fusion pilot")
    print(f"Device: {DEVICE}")
    print("Prompt policy: baseline label-only, iKnow uses pure single-hop concat without Top-P, selective hop1/hop2 use filtered acoustic LLM prompts with concat fallback; final fusion uses sigmoid-gated MoE over baseline, hop1, and hop2")
    ...
    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="ESC50 sigmoid MoE"):
        audio_path = sample["audio_path"]
        true_indices = sample["true_indices"]
        if not os.path.exists(audio_path) or not true_indices:
            skipped_count += 1
