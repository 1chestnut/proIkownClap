...
OUTPUT_JSON = "/data/zkx/zkx/iknow-audio/25.gmu_residual/tut2017/results_gmu_residual.json"
...
def gmu_gate_from_gain(gain_value, q25, q75):
    center = 0.5 * (q25 + q75)
    scale = max(0.5 * (q75 - q25), 1e-6)
    return float(1.0 / (1.0 + np.exp(-(gain_value - center) / scale)))
...
def build_selective_originalagg_score(clap_model, audio_embed, cos_sim_orig, ci, label_classes, kg_classes, class_labels_set, hop1_relations, hop2_relations, get_tails_fn, prompt_map):
    class_name = label_classes[ci]
    kg_ent = get_kg_entity(kg_classes[ci])
    tau = cos_sim_orig[ci].item() + RELATIVE_MARGIN
    h1_map = build_h1_map(kg_ent, class_name, class_labels_set, hop1_relations, get_tails_fn, prompt_map)
    ...
    if s1.numel() > 0 and max_h1 >= tau:
        best_scores = safe_topk(s1, TOP_P)
        if best_scores.numel() == 0:
            return None, len(h1_map), False
        lse_inputs = torch.cat([cos_sim_orig[ci].unsqueeze(0) * LOGIT_SCALE, best_scores * LOGIT_SCALE])
        score_value = (torch.logsumexp(lse_inputs, dim=0) - np.log(lse_inputs.numel())) / LOGIT_SCALE
        return score_value, len(h1_map), False
    ...
    best_scores = safe_topk(all_scores, TOP_P)
    if best_scores.numel() == 0:
        return None, len(h1_map) + len(h2_prompts), True
    lse_inputs = torch.cat([cos_sim_orig[ci].unsqueeze(0) * LOGIT_SCALE, best_scores * LOGIT_SCALE])
    score_value = (torch.logsumexp(lse_inputs, dim=0) - np.log(lse_inputs.numel())) / LOGIT_SCALE
    return score_value, len(h1_map) + len(h2_prompts), True
...
def run_selective2hop_gmu_residual(clap_model, audio_embed, cos_sim_orig, top_indices, label_classes, kg_classes, class_labels_set, hop1_relations, hop2_relations, get_tails_fn, prompt_map, q25, q75):
    score = cos_sim_orig.clone()
    prompt_count = 0
    hop2_flags = []
    gate_values = []
    for ci in top_indices:
        originalagg_s, used_prompts, hop2_flag = build_selective_originalagg_score(
        ...
        if originalagg_s is None:
            gate_values.append(0.0)
            continue
        gain_value = float((originalagg_s - cos_sim_orig[ci]).item())
        gate_i = gmu_gate_from_gain(gain_value, q25, q75)
        gate_values.append(gate_i)
        positive_gain = max(0.0, gain_value)
        score[ci] = cos_sim_orig[ci] + gate_i * positive_gain
    extras = {
        "hop2_activated": any(hop2_flags),
        "candidate_level_activation_rate": float(np.mean(hop2_flags)) if hop2_flags else 0.0,
        "alpha_mean": float(np.mean(gate_values)) if gate_values else 0.0,
    }
    return score, prompt_count, extras
...
@torch.no_grad()
def collect_gain_quantiles(clap_model, dataset, text_embeds, label_classes, kg_classes, class_labels_set, hop1_relations, hop2_relations, get_tails, prompt_map):
    gains = []
    skipped = 0
    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="Collect TUT candidate gains"):
        ...
        for ci in top_indices:
            originalagg_s, _, _ = build_selective_originalagg_score(
                clap_model, audio_embed, cos_sim_orig, ci, label_classes, kg_classes,
                class_labels_set, hop1_relations, hop2_relations, get_tails, prompt_map
            )
            if originalagg_s is not None:
                gains.append(float((originalagg_s - cos_sim_orig[ci]).item()))
    q25 = float(np.quantile(gains, 0.25)) if gains else 0.0
    q75 = float(np.quantile(gains, 0.75)) if gains else 1.0
    return q25, q75, skipped


def print_final_tables(results):
    metrics = {name: compute_metrics(results[name]["ranks"]) for name in results}
    order = ["Baseline", "iKnow", "Full2Hop", "Selective2Hop_OriginalAgg", "Selective2Hop_StaticAlpha", "Selective2Hop_GMUResidual"]
    print("\n" + "=" * 180)
    print(f"{'Metric':<8} | {'Baseline':<10} | {'iKnow':<10} | {'Full2Hop':<12} | {'Sel2 OriginalAgg':<18} | {'Sel2 StaticAlpha':<18} | {'Sel2 GMUResidual':<18}")
    print("-" * 180)
...
@torch.no_grad()
def main():
    print("Starting TUT2017 GMU-style residual fusion pilot")
    print(f"Device: {DEVICE}")
    print("Prompt policy: baseline label-only, iKnow uses pure single-hop concat without Top-P, selective hop1/hop2 use LLM prompts with concat fallback; final fusion uses GMU-style gated positive residual over OriginalAgg")
    ...
    q25, q75, gain_skipped = collect_gain_quantiles(
        clap_model, dataset, text_embeds, label_classes, kg_classes,
        class_labels_set, hop1_relations, hop2_relations, get_tails, prompt_map
    )
    print(f"OriginalAgg gain quantiles for GMU residual fusion: q25={q25:.6f}, q75={q75:.6f}, skipped={gain_skipped}")
    results = init_results()
    skipped_count = 0

    for sample in tqdm(iter_samples(dataset), total=dataset["total"], desc="TUT2017 candidate gain"):
        audio_path = sample["audio_path"]
        true_indices = sample["true_indices"]
        if not os.path.exists(audio_path) or not true_indices:
            skipped_count += 1
        ...
        t = time.time()
        score, prompt_count, extras = run_selective2hop_gmu_residual(
            clap_model, audio_embed, cos_sim_orig, top_indices, label_classes, kg_classes,
            class_labels_set, hop1_relations, hop2_relations, get_tails, prompt_map, q25, q75
        )
        results["Selective2Hop_GMUResidual"]["ranks"].append(rank_of_true(score, true_indices))
        results["Selective2Hop_GMUResidual"]["times"].append(audio_ms + (time.time() - t) * 1000.0)
        results["Selective2Hop_GMUResidual"]["prompts"].append(prompt_count)
        results["Selective2Hop_GMUResidual"]["hop2_activation_sample"].append(bool(extras["hop2_activated"]))
        results["Selective2Hop_GMUResidual"]["hop2_activation_candidate"].append(float(extras["candidate_level_activation_rate"]))
        results["Selective2Hop_GMUResidual"]["alphas"].append(float(extras["alpha_mean"]))
