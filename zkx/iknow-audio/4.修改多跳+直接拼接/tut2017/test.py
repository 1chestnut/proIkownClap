...
TOP_P = 5       
ALPHA = 0.6          # 🌟 固定类锚定最优权重
DECAY_GAMMA = 0.85   # 🌟 二跳距离衰减系数 (打压场景漂移)
LOGIT_SCALE = 100.0  
...
@torch.no_grad()
def main():
    print(f"🚀 启动 TUT2017 M³KG-RAG 机制验证: 多跳关系衰减与聚合函数同台竞技...")
    ...
    # 💡 结构层修复：TUT是场景识别，极易发生“场景->物体->其他场景”的灾难性漂移
    ALL_RELS = ['is variant of', 'has parent', 'scene contains', 'event composed of', 'described by']
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="M³KG-RAG Testing"):
        ...
        for c_idx in top_k_indices:
            ...
            # 记录实体与是否为二跳：dict[tail_name, is_hop2_boolean]
            tail_hop_map = {}
            
            # --- 挖掘多跳场景知识 ---
            for r1 in HOP1_RELATIONS:
                hop1_tails = get_top_m_tails(kg_scene_name, r1, TOP_M)
    ...
    print("\n💡 TUT2017 测试解读:")
    print("场景分类非常容易在二跳发生【环境漂移】(如从 城市中心 -> 汽车 -> 住宅区)。")
    print("M3 的 0.85 衰减率，能有效确保那些在空间逻辑上跳跃过大的实体在剪枝阶段就被淘汰，留下真正辅助场景辨识的关键声音事件！")
