...
# 🔥 新增：负向知识惩罚系数 (专门对付AudioSet/FSD50K的混淆类)
BETA_NEG = 0.3  
...
@torch.no_grad()
def main():
    print(f"🚀 启动 FSD50K: [负向剥离 BETA={BETA_NEG}] + [静态早退 {RELATIVE_MARGIN}] + [TOP_K={TOP_K}]")
    ...
    for _, row in tqdm(eval_df.iterrows(), total=len(eval_df), desc="Adaptive M3 Pipeline"):
        fname = str(row['fname'])
        ...
        for c_idx in top_k_indices:
            ...
            if max_hop1_score >= tau_dynamic:
                ...
                if len(hop2_prompts) > 0:
                    ...
                    # 🔥 对 Hop-2 知识也进行负向剥离 (给个0.5的衰减，防止减太多)
        ...
        ranks['Ours_AdaptiveM3'].append(min([np.where(sorted_indices_kg == t_idx)[0][0] + 1 for t_idx in true_indices]))
